...
export const GrowthSummary = ({ accounts }: { accounts: any[] }) => {
  ...
  const weeklyPercentage = totalFollowers > 0 
    ? (totalWeeklyGrowth / (totalFollowers - totalWeeklyGrowth)) * 100 
    : 0;
  
  const monthlyPercentage = totalFollowers > 0 
...
export const PlatformDistribution = ({ accounts }: { accounts: any[] }) => {
  ...
  const platformData = Object.values(SocialMediaPlatform).map(platform => {
    ...
    return {
      ...
      color: getPlatformColor(platform as SocialMediaPlatform).replace('bg-', '')
    };
...
export const FollowerHistoryChart = ({ 
  ...
  // Add current followers if not in history
  if (historyData.length === 0 || 
      new Date(historyData[historyData.length - 1].date).getTime() !== new Date(account.lastUpdated).getTime()) {
    historyData.push({
