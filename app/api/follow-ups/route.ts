...
export async function GET(req: NextRequest) {
  try {
    ...
    const isAdminOrPastor = ["Admin", "Pastor", "HQ_ADMIN", "CENTER_ADMIN"].some(role => userRoles.includes(role));
    ...
    const transformedFollowUps = followUps.map(followUp => {
      ...
      return {
        ...
        personType: followUp.personType,
        personName,
        personEmail: populatedPerson?.email || followUp.newAttendee?.email,
        personPhone: populatedPerson?.phoneNumber || followUp.newAttendee?.phoneNumber,
        personWhatsApp: populatedPerson?.whatsappNumber || followUp.newAttendee?.whatsappNumber,
        status: followUp.status,
        responseCategory: followUp.responseCategory,
        assignedTo: populatedAssignedTo ? {
          _id: populatedAssignedTo._id.toString(),
